...
class Controller_Admin
{
    ...
    public function get_dashboard_info()
    {
        ...
        $class->actionlink = "/index.php?action=show_classes_panel";
        $subject = new stdclass();
        $subject->count = $model->get_total_subject();
        $subject->name = "Môn Học";
        $subject->icon = "fa-book";
        $subject->actionlink = "/index.php?action=show_subjects_panel";
        $question = new stdclass();
        $question->count = $model->get_total_question();
        $question->name = "Câu Hỏi";
        $question->icon = "fa-question";
        $question->actionlink = "/index.php?action=show_questions_panel";
        $test = new stdclass();
        $test->count = $model->get_total_test();
        $test->name = "Bài Thi";
        $test->icon = "fa-edit";
        $test->actionlink = "/index.php?action=show_tests_panel";
    ...
    public function check_del_teacher()
    {
        ...
        if ($del) {
            ...
            $result['status_value'] = "Không thể xóa giảng viên đang chủ nhiệm lớp!";
            $result['status'] = 0;
            $result['teacher_id'] = $teacher_id;
        }
        echo json_encode($result);
    ...
    public function check_del_class()
    {
        ...
        if ($del) {
            ...
            $result['status_value'] = "Không thể xóa lớp đang có sinh viên!";
            $result['status'] = 0;
            $result['class_id'] = $class_id;
        }
        echo json_encode($result);
    ...
    public function check_del_student()
    {
        ...
        if ($del) {
            ...
            $result['status_value'] = "Không thể xóa sinh viên đang có điểm trên hệ thống!";
            $result['status'] = 0;
            $result['student_id'] = $student_id;
        }
        echo json_encode($result);
    ...
    public function delete_check_teachers()
    {
        ...
        if ($list_del == '') {
            ...
            $result['status'] = 0;
            $result['status_value'] = "Không thể xóa ID: ".$list_del.", Giảng viên đang chủ nhiệm lớp!";
        }
        echo json_encode($result);
    }
    public function delete_check_classes()
    {
        ...
        if ($list_del == '') {
            ...
            $result['status'] = 0;
            $result['status_value'] = "Không thể xóa ID: ".$list_del.", lớp đang có sinh viên!";
        }
        echo json_encode($result);
    }
